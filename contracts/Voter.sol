...
contract Voter{
    ...
    event VoteCast(address voter, address candidate, uint96 votesCast, string reason);
    ...
    function initialise(uint256 launchPhase, uint256 votePeriod, address govTokenAddress) external returns(uint256, uint256, uint256){
    ...
    function castVote(address newGovernor) external {
        emit VoteCast(msg.sender, newGovernor, castVoteInternal(msg.sender, newGovernor), "");
    }

    function castVoteWithReason(address newGovernor, string calldata reason) external {
        emit VoteCast(msg.sender, newGovernor, castVoteInternal(msg.sender, newGovernor), reason);
    }


    function castVoteInternal(address voter, address newGovernor) internal returns (uint96) {
        require(block.number > voteStart && block.number < voteEnd, "castVoteInternal: voting is closed");
    ...
    function executeTransaction(address target, uint value, string memory signature, bytes memory data) public payable returns (bytes memory) {
        require(msg.sender == winner, "executeTransaction: Call must come from governance");
